...
## 11. Design System & Tema Global (UI/UX)
- **Tema Warna Utama (Premium Earth):**
  - **Primary (Utama):** `#2F5C36` (Hijau Daun Tua) - Digunakan untuk tombol utama, header, dan elemen interaktif.
  - **Secondary (Sekunder):** `#6B4F3A` (Cokelat Tanah Elegan) - Digunakan untuk aksen sekunder, teks penegas, atau ikon pendukung.
  - **Background (Latar):** `#FFFFFF` (Putih Murni) - Latar belakang utama aplikasi agar tampilan bersih dan modern.
  - **Muted/Accent (Hover):** `#F9F8F6` (Krem Sangat Muda) - Latar belakang saat hover (seperti pada menu) atau untuk kartu (card) sekunder.
- **Tipografi (Font):** Gunakan keluarga font *sans-serif* modern standar (seperti `Inter`, `Roboto`, atau `Geist`). Jaga hierarki visual dengan ketebalan yang tegas (`font-bold` / `font-semibold` untuk judul, dan `text-muted-foreground` untuk deskripsi tambahan).
- **Bentuk Visual:** Gunakan sudut melengkung moderat (`rounded-md` atau `rounded-lg`) pada tombol, kartu, dan menu untuk kesan ramah dan organik. Hindari sudut yang terlalu lancip (`rounded-none`) kecuali diperlukan.

## 12. Tema Spesifik Projek (Desa Bebas Sampah)
- **Fokus Utama:** Sistem Bank Sampah, proses daur ulang, dan edukasi kebersihan warga.
- **Konsep Visual:** Memvisualisasikan program kebersihan dengan palet warna alam (Hijau Daun, Cokelat Tanah, Biru Muda).
- **Elemen Kunci UI/UX:**
  - Hero image menampilkan ruang terbuka hijau / aktivitas lingkungan.
  - Terdapat Dashboard sistem Bank Sampah.
  - Panduan daur ulang interaktif.
  - Jadwal edukasi kebersihan warga.
